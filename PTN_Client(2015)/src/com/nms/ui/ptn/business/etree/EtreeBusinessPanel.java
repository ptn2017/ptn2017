 ...
 * @author lepan
 */
public class EtreeBusinessPanel extends ContentView<EtreeInfo> {
	/**
	 * 
	 */
	...
	public EtreeBusinessPanel() {
		super("etreeBusinessTable",RootFactory.CORE_MANAGE);
		try {
		init();
	...
	public void init() {
		getContentPanel().setBorder(BorderFactory.createTitledBorder(ResourceUtil.srcStr(StringKeysTab.TAB_ETREEINFO)));
		initComponent();
		setLayout();
		setActionListention();
	}
	...
	private void setActionListention() {
		...
		activeItem.addActionListener(new MyActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent arg0) {
				((EtreeBusinessController) getController()).doActive();
			}
		...
		unActiveItem.addActionListener(new MyActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent arg0) {
				((EtreeBusinessController) getController()).doUnActive();
			}
	...
	public void setController() {
		controller = new EtreeBusinessController(this);
	}
