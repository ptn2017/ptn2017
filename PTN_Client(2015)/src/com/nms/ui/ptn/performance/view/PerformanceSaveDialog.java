...
public class PerformanceSaveDialog extends PtnDialog {
	/**
	 * 
	 */
	...
	private PerformanceSaveController unloadController=null;
	...
	public PerformanceSaveDialog(PerformanceSaveController unloadController) {
		try {
		super.setTitle("性能数据存储管理");
	...
	private void addListener() {		
		...
		cancel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				PerformanceSaveDialog.this.dispose();
			}
		});	
	...
	private void initComponents() throws Exception {
		...
		if(unload.getLogType()==3 && unload.getFileWay().equals("")){
			...
			unload.setFileVWay(ServerConstant.AUTODATABACKPM_FILE);
		...
		}else if(unload.getLogType()==8 && unload.getFileWay().equals("")){
			unload.setFileVWay(ServerConstant.AUTODATABACKPM_FILE);
		}else if(unload.getLogType()==8 && unload.getFileVWay().equals("")){
			unload.setFileVWay(ServerConstant.AUTODATABACKPM_FILE);
		}else if(unload.getLogType()==9 && unload.getFileWay().equals("")){
			unload.setFileVWay(ServerConstant.AUTODATABACKPM_FILE);
		}else if(unload.getLogType()==9 && unload.getFileVWay().equals("")){
			unload.setFileVWay(ServerConstant.AUTODATABACKPM_FILE);
		}
...
//		fileVFiled.setEditable(false);//只读
		...
		volumeFiled.setText("500");
		volumeJ= new JLabel(ResourceUtil.srcStr(StringKeysLbl.LBL_LOG_MAXVOLUMN_M));//开始时间
		timeInterval = new JLabel("时间间隔(天)");//时间间隔
		timeJSpinner = new PtnSpinner(unload.getTimeLimit(),60,Integer.MAX_VALUE,1);
...
//				  if(Integer.parseInt(volumeFiled.getText().trim()) < 500){
//					  DialogBoxUtil.succeedDialog(null,  "容量大于等于500M");
//					  return;
//				  }
...
//					}
	...
	private void startThrad(LogManager unload){
		new PerformanceRamTimerTask().dispathPerformRam(unload);
