...
public class CamporeBusinessInfoLoader implements LazyLoader {
	 ...
	 * @param neLists 网元数据
	 */
	public CamporeBusinessInfoLoader(TDataBox box, Object emsMap, Object neMap, CamporeBusinessDataDialog camporeDataDialog) {
	...
	public void camporeData(boolean isEqual,Object emsMap, Object neMap){
		...
		Object busiObject = null;
			Map<Integer, List> eMSMap = (Map<Integer, List>) emsMap;
...
//				this.camporeServiceElement(emslist, nelist, 52);
	 ...
	 * @param neTunnels
	 */
	private void camporeTunnelElement(Map<Integer, List> tunnelEMSMap, Map<Integer, List> tunnelNEMap){
		Map<Integer, List<Integer>> tunnelIdMap = new HashMap<Integer, List<Integer>>();//所有tunnel业务id集合
		Map<Integer, List<Integer>> protectIdMap = new HashMap<Integer, List<Integer>>();//所有保护id集合
		HashMap<Integer, HashMap<Integer, Tunnel>> emsProtectHashMap = new HashMap<Integer, HashMap<Integer, Tunnel>>();
		HashMap<Integer,HashMap<Integer, Tunnel>> neProtectHashMap = new HashMap<Integer, HashMap<Integer, Tunnel>>();
		HashMap<Integer, HashMap<Integer, Tunnel>> emsTunnelHashMap = this.getHashMapTunnnel(tunnelEMSMap, tunnelIdMap,protectIdMap,emsProtectHashMap);
		HashMap<Integer, HashMap<Integer, Tunnel>> neTunnelHashMap = this.getHashMapTunnnel(tunnelNEMap, tunnelIdMap,protectIdMap,neProtectHashMap);
//		Collections.sort(tunnelIdMap);
//		Collections.sort(protectIdMap);
	 ...
	 * @param neTunnels
	 */
	private void camporePwElement(Map<Integer, List> pwEMSMap, Map<Integer, List> pwNEMap){
		Map<Integer, List<Integer>> pwInfoIdMap = new HashMap<Integer, List<Integer>>();//所有的pwinfo业务id集合
//		List<Integer> msPwIds = new ArrayList<Integer>();//所有的多段pw业务集合
		HashMap<Integer, HashMap<Integer, PwInfo>> emsPwHashMap = new HashMap<Integer, HashMap<Integer, PwInfo>>();
...
//			}
  ...
  * @param pwInfo2
  */
	private void createPwElement(PwInfo emsPwInfo1, PwInfo nePwInfo1, int siteId) {
		PwInfo emsPwInfo = null;
		PwInfo nePwInfo = null;
		int label =1;
		try {
			...
			if(emsPwInfo != null){
				...
				createchildNode(ResourceUtil.srcStr(StringKeysLbl.LBL_ROLE),
						    role,
						    emsPwInfo.getASiteId() == siteId?"A":"Z",
						    elements,label);
				//模式
	 ...
	 * @param pwInfoIdMap
	 */
	private void getHashMapPw(Map<Integer, List> pwEMSMap, HashMap<Integer, 
			HashMap<Integer, PwInfo>> emsPwHashMap, Map<Integer, List<Integer>> pwInfoIdMap) {
		 try {
			 for(Integer siteId : pwEMSMap.keySet()){
				 List<PwInfo> pwList = pwEMSMap.get(siteId);
				 HashMap<Integer, PwInfo> pwMap = new HashMap<Integer, PwInfo>();
				 List<Integer> pwIdList = new ArrayList<Integer>();
				 for(PwInfo pw : pwList){
					 if(pw.getASiteId() == siteId){
						 setPwHashMap(pwMap, pwIdList, pw.getApwServiceId(), pw);
					 }else if(pw.getZSiteId() == siteId){
						 setPwHashMap(pwMap, pwIdList, pw.getZpwServiceId(), pw);
					 }
				 }
				 emsPwHashMap.put(siteId, pwMap);
				 pwInfoIdMap.put(siteId, pwIdList);
			 }
		} catch (Exception e) {
			ExceptionManage.dispose(e, getClass());
		}
...
//	}
	 ...
	 * @param pwInfo
	 */
	private void setPwHashMap(HashMap<Integer, PwInfo> emsPwHashMap, List<Integer> pwIdList, int pwId, PwInfo pwInfo) {
		emsPwHashMap.put(pwId, pwInfo);
		if(!pwIdList.contains(pwId)){
			pwIdList.add(pwId);
		}
	}
	 ...
	 * @param siteId 
	 */
	private void createTunnelElement(Tunnel tunnelEMS,Tunnel tunnelNE, Integer id, Integer siteId){
		Node node = new Node();
		node.setName("TUNNEL");
		...
		try {
			...
			if(tunnelNE != null){//网管存在，设备不存在的数据
				...
				for(OamInfo oamInfo : tunnelNE.getOamList()){
					if(oamInfo.getOamMep() != null){
						if(oamInfo.getOamMep().getSiteId() == siteId){
						...
						if(oamInfo.getOamMip().getSiteId() == siteId){
				...
				}else if(tunnelNE.getzSiteId() == siteId){
			...
			if(tunnelEMS != null){//设备存在，网管不存在的数据
				...
				PortInst portinst = new PortInst();
				PortInst inst = new PortInst();
				if(tunnelEMS.getaSiteId() == siteId){
				...
				}else if(tunnelEMS.getzSiteId() == siteId){
	 ...
	 * @return
	 */
	private HashMap<Integer, HashMap<Integer, Tunnel>> getHashMapTunnnel(Map<Integer, List> tunnelEMSMap, Map<Integer, List<Integer>> idMap,
			Map<Integer, List<Integer>> proetctIdMap, HashMap<Integer,HashMap<Integer, Tunnel>> hashMap){
		HashMap<Integer, HashMap<Integer, Tunnel>> tunnelMap = new HashMap<Integer, HashMap<Integer, Tunnel>>();
		for(Integer siteId : tunnelEMSMap.keySet()){
			List<Tunnel> tunnelList = tunnelEMSMap.get(siteId);
			List<Integer> ids = new ArrayList<Integer>();
			List<Integer> proetctIds = new ArrayList<Integer>();
			HashMap<Integer, Tunnel> tMap = new HashMap<Integer, Tunnel>();
			HashMap<Integer, Tunnel> ptMap = new HashMap<Integer, Tunnel>();
			for(Tunnel tunnel : tunnelList){
				if(tunnel.getLspParticularList().size() ==1){
					if(tunnel.getaSiteId() == siteId){
						setHashMap(tMap, ids, tunnel.getLspParticularList().get(0).getAtunnelbusinessid(), tunnel);
					}else if(tunnel.getzSiteId() == siteId){
						setHashMap(tMap, ids, tunnel.getLspParticularList().get(0).getZtunnelbusinessid(), tunnel);
					}
				}else if(tunnel.getLspParticularList().size() == 2){
					if(tunnel.getLspParticularList().get(0).getASiteId() == siteId){
						setHashMap(tMap, ids,tunnel.getLspParticularList().get(0).getAtunnelbusinessid(), tunnel);
					}else if(tunnel.getLspParticularList().get(0).getZSiteId() == siteId){
						setHashMap(tMap, ids,tunnel.getLspParticularList().get(0).getZtunnelbusinessid(), tunnel);
					}else if(tunnel.getLspParticularList().get(1).getASiteId() == siteId){
						setHashMap(tMap, ids,tunnel.getLspParticularList().get(1).getAtunnelbusinessid(), tunnel);
					}else if(tunnel.getLspParticularList().get(1).getZSiteId() == siteId){
						setHashMap(tMap, ids,tunnel.getLspParticularList().get(1).getZtunnelbusinessid(), tunnel);
					}
				}
				if(tunnel.getProtectTunnel() != null){
					if(tunnel.getProtectTunnel().getaSiteId() == siteId){
						if(!proetctIds.contains(tunnel.getProtectTunnel().getAprotectId())){
							proetctIds.add(tunnel.getProtectTunnel().getAprotectId());
						}
						ptMap.put(tunnel.getProtectTunnel().getAprotectId(), tunnel);
					}else if(tunnel.getProtectTunnel().getzSiteId() == siteId){
						if(!proetctIds.contains(tunnel.getProtectTunnel().getZprotectId())){
							proetctIds.add(tunnel.getProtectTunnel().getZprotectId());
						}
						ptMap.put(tunnel.getProtectTunnel().getZprotectId(), tunnel);
					}
				}
			}
			tunnelMap.put(siteId, tMap);
			idMap.put(siteId, ids);
			proetctIdMap.put(siteId, proetctIds);
			hashMap.put(siteId, ptMap);
		}
...
//			}
	 ...
	 * @param type 
	 */
	private void camporeServiceElement(Map<Integer, List> eMSMap, Map<Integer, List> nEMap, int type) {
		Map<Integer, List<Integer>> serviceIdMap = new HashMap<Integer, List<Integer>>();
		Map<Integer, HashMap<Integer, ServiceInfo>> emsMap = new HashMap<Integer, HashMap<Integer, ServiceInfo>>();
		Map<Integer, HashMap<Integer, ServiceInfo>> neMap = new HashMap<Integer, HashMap<Integer, ServiceInfo>>();
		this.getServiceMap(eMSMap, emsMap, serviceIdMap, type);
		this.getServiceMap(nEMap, neMap, serviceIdMap, type);
	...
	private void getServiceMap(Map<Integer, List> map, Map<Integer, HashMap<Integer, ServiceInfo>> serviceInfoMap, 
			Map<Integer, List<Integer>> serviceIdMap, int type) {
		for(Integer siteId : map.keySet()){
			...
			for(ServiceInfo serviceInfo : (List<ServiceInfo>)map.get(siteId)){
				int serviceId = 0;
				if(type == 0){
					if(serviceInfo.getaSiteId() == siteId){
					...
					if(((EtreeInfo)serviceInfo).getRootSite() == siteId){
			...
			serviceIdMap.put(siteId, serviceIdList);
			serviceInfoMap.put(siteId, serviceMap);
		}
	}
	 ...
	 * @param type 
	 */
	private void createServiceElement(ServiceInfo serviceEms, ServiceInfo serviceNe, int type, int siteId) {
		...
		if(type == 3){
			...
			String neName = serviceNe == null ? null:this.getServiceType(((EtreeInfo)serviceNe), siteId);
			serviceTypeData.setNE(neName);
			String emsName = serviceEms == null ? null:this.getServiceType((EtreeInfo)serviceEms, siteId);
...
//	}
	...
	private String getServiceType(EtreeInfo etreeInfo, int siteId) {
		if(etreeInfo.getRootSite() > 0 && etreeInfo.getRootSite() == siteId){
			return ResourceUtil.srcStr(StringKeysObj.ROOT);
		}else if(etreeInfo.getBranchSite() > 0 && etreeInfo.getBranchSite() == siteId){
