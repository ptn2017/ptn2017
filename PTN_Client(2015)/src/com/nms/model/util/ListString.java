		...
		try {
			if(list.size()>0){
				...
				if(object instanceof SSAlarm){    //导出告警统计
					...
					for(int i=list.size()-1;i>=0;i--){
						beanData=new String[8];
						OperationLog operationLog=(OperationLog) list.get(i);
						beanData[0]=operationLog.getUserName();
						beanData[1]=operationLog.getOperationObjName();
						beanData[2]=siteService.getSiteName(operationLog.getSiteId());
						beanData[3]=operationLog.getStartTime();
						beanData[4]=operationLog.getOverTime();
						beanData[5]=EOperationLogType.forms(operationLog.getOperationType()).toString();
						String resultName="";
