...
public class DualInfoService_MB extends ObjectService_Mybatis{
	...
	public Map<Integer, List<DualInfo>> select(int label,int siteId) throws Exception {
		...
		try {
			...
			UiUtil.closeService_MB(pwProtetcServiceMB);
			 pwProtect = null;
			 pwProtectList = null;
	 ...
	 * 根据pwId查询dual的名称
	 */
	public String queryNameByPwId(int pwId){
		String name = null;
		try {
			List<DualInfo> dualList = dualInfoMapper.queryNameByPwId(pwId);
			if(dualList != null && dualList.size() > 0){
				name = dualList.get(0).getName();
			}
		} catch (Exception e) {
			ExceptionManage.dispose(e, this.getClass());
		}
