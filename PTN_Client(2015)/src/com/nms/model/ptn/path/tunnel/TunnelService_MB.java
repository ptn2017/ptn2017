...
public class TunnelService_MB extends ObjectService_Mybatis {
	...
	public int savaProtect(Tunnel protectTunnel) throws Exception {
		...
		try {
			...
			tunnelId = tunnelMapper.insert(protectTunnel);
			// 添加倒换命令
	...
	public void getLabel(Lsp lsp, int isSingle, SiteService_MB siteServiceMB, LabelInfoMapper labelInfoMapper, LabelInfoService_MB labelInfoServiceMB) throws Exception {
		...
		try {
			...
			if (isSingle == 0) {
				...
				labelInfoServiceMB.getSqlSession().commit();
			} else {
	...
	public Businessid setbusinessId(Lsp lspParticular, Map<Integer, Integer> siteServicemap, BusinessidService_MB businessidService_MB, String type) throws Exception {
		...
		try {
			...
			/** 获取A端的tunnel业务id */
			if (businessId == 0) {
				businessid_tunnel = businessidService_MB.getIdValue(siteId, "tunnel");
			} else {
	...
	// 查询并设置OAM、QoS、保护信忿
	public void setOtherInfomationforTunnel(List<Tunnel> tunnels) {
		OamInfoService_MB oamInfoServiceMB = null;
		...
		try {
			...
			for (Lsp obj : lspparticularlist) {
				if (tunnel.getIsSingle() == 0) { // 修改标签状态，如果是网络配置，修改a z端否则只修改一端
					...
					labelInfoService_MB.updateBatch(obj.getBackLabelValue(), obj.getASiteId(), 1, "TUNNEL");
					labelInfoService_MB.updateBatch(obj.getFrontLabelValue(), obj.getZSiteId(), 1, "TUNNEL");
			...
			for (Lsp obj : lspparticularlist) {
				...
				if (tunnel.getIsSingle() == 0) {
					...
					labelInfoService_MB.insertNewLabel(obj.getFrontLabelValue(), obj.getZSiteId(), "TUNNEL");
					labelInfoService_MB.insertNewLabel(obj.getBackLabelValue(), obj.getASiteId(), "TUNNEL");
					// labelInfoDao.insertNewLabel(obj.getBackLabelValue(),
					// obj.getZSiteId(), connection);
					// labelInfoDao.updateStatusBatch(obj.getFrontLabelValue(),
					// obj.getASiteId(), 0, connection);
					labelInfoService_MB.updateBatch(obj.getFrontLabelValue(), obj.getZSiteId(), 0, "TUNNEL");
					labelInfoService_MB.updateBatch(obj.getBackLabelValue(), obj.getASiteId(), 0, "TUNNEL");
					// labelInfoDao.updateStatusBatch(obj.getBackLabelValue(),
	...
	private void deleteProtect(Tunnel tunnel) throws Exception {
		...
		try {
			...
			qosRelevancemapper.delete(qosRelevance);
	 ...
	 * @throws Exception
	 */
	public void setOnlyTunnelLsp(List<Tunnel> tunnelList, boolean isNode) throws Exception {
		Lsp lspParticular = null;
		LspinfoMapper lspParticularMapper = sqlSession.getMapper(LspinfoMapper.class);
		List<Lsp> lspparticularList = null;
		List<Lsp> lspparticularList1 = null;
	...
	public void doSearchWh(List<Tunnel> tunnels) {
		...
		List<Integer> integers = new ArrayList<Integer>();
		for (Tunnel tunnel : tunnels) {
			integers.add(tunnel.getTunnelId());
		}
		Map<String, Object> map = new HashMap<String, Object>();
		...
		SegmentMapper segmentMapper = null;
		LspinfoMapper lspinfoMapper = null;
		try {
			segmentMapper = this.sqlSession.getMapper(SegmentMapper.class);
			if (tunnels.size() == 2) {
				// 1 插入新tunnel 表 数据
				tunnelMapper.insert_searchWH(tunnels.get(0).getTunnelId(), tunnels.get(1).getTunnelId(), newTunnelName);
				newTunnelId = tunnelMapper.selectTunnelIdByTunnelName(newTunnelName).get(0);
				...
				lspWhSearchdo(newTunnelId, lsps, segmentMapper);
			} else {
	...
	private void lspWhSearchdo(int newTunnelId, List<Lsp> lsps, SegmentMapper segmentMapper) throws Exception{
		LspinfoMapper lspinfoMapper = null;
		lspinfoMapper = sqlSession.getMapper(LspinfoMapper.class);
		try {
			List<Segment> segmentList = segmentMapper.query_search(lsps.get(0).getAPortId(), lsps.get(1).getZPortId());
			if(segmentList != null && segmentList.size() == 1){
				lsps.get(0).setSegmentId(segmentList.get(0).getId());
			}
		} catch (Exception e) {
	...
	public static void main(String[] args) {
		try {
			...
			tunnel.setaPortId(14);
			List<Tunnel> tunnels = tunnelServiceMB.filterSelectNE(4, tunnel);
	...
	public Tunnel queryTunnelByName(String tunnelName) {
		try {
			List<Tunnel> tList = this.tunnelMapper.queryTunnelByName(tunnelName);
